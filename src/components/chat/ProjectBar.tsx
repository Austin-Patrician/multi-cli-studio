...
import { DesktopWindowControls } from "../DesktopWindowChrome";
import { LaunchScriptButton } from "./LaunchScriptButton";
import { OpenWorkspaceMenu } from "./OpenWorkspaceMenu";
import { WORKSPACE_PANEL_MODES, type WorkspacePanelMode } from "./workspacePanelModes";
        ...
        <div className="min-w-0 flex-1 px-4 py-2.5">
          <div className="flex min-w-0 items-center justify-between gap-3">
            ...
            <div className="flex shrink-0 items-center gap-1.5">
              ...
              {rightPanelCollapsed ? <DesktopWindowControls /> : null}
            </div>
          </div>
        </div>
        {!rightPanelCollapsed ? (
          <div className="flex w-[380px] min-w-[340px] items-center justify-between gap-3 border-l border-border px-3 py-2.5">
            <div className="file-tree-tabs-wrap min-w-0">
              <div className="panel-tabs">
                {WORKSPACE_PANEL_MODES.map((item) => {
                  const Icon = item.icon;
                  const isActive = panelMode === item.id;
                  return (
                    <button
                      key={item.id}
                      type="button"
                      onClick={() => onSelectPanelMode(item.id)}
