...
use crate::{resolve_workspace_target, AppStore, WorkspaceTarget};
...
fn resolve_local_workspace_root(store: &AppStore, workspace_id: &str) -> Result<PathBuf, String> {
    let target = resolve_workspace_target(store, Some(workspace_id), None)?;
    match target {
        WorkspaceTarget::Local { project_root } => PathBuf::from(project_root)
            .canonicalize()
            .map_err(|err| format!("Failed to resolve workspace root: {err}")),
        WorkspaceTarget::Ssh { .. } => {
            Err("Code intelligence is currently available only for local workspaces".to_string())
        }
    }
}
...
pub async fn code_intel_definition(
    ...
    let workspace_root = resolve_local_workspace_root(&store, &workspace_id)?;
